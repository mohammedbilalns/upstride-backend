...
export class HandleRescheduleUC implements IHandleRescheduleUC {
    ...
    async execute(dto: HandleRescheduleDto): Promise<Booking> {
        ...
        if (!currentSlot) throw new AppError("Current slot info missing", HttpStatus.INTERNAL_SERVER_ERROR);
        ...
        if (!booking.rescheduleRequest || booking.rescheduleRequest.status !== "PENDING") {
            throw new AppError("No pending reschedule request", HttpStatus.BAD_REQUEST);
        }
        ...
        if (dto.action === "REJECTED") {
            ...
            if (!updated) throw new AppError("Failed to update booking", HttpStatus.INTERNAL_SERVER_ERROR);
            return updated;
        }
        ...
        if (!newSlot || newSlot.status !== SlotStatus.OPEN) {
            throw new AppError("New slot is no longer available", HttpStatus.CONFLICT);
        }
