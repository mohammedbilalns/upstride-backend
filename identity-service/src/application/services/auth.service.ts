...
import { buildOtpEmailHtml, OTP_SUBJECT } from "../utils/otp";
...
export class AuthService implements IAuthService {
  ...
  async registerUser(name: string, email: string, password: string): Promise<void> {
		...
		if(existingUser && existingUser?.isVerified) throw new AppError(ErrorMessage.ALERADY_WITH_GOOGLE_ID, HttpStatus.BAD_REQUEST)
		if(existingUser && !existingUser?.isVerified) await this._userRepository.delete(existingUser.id)
		const hashedPassword = await this._cryptoService.hash(password); 
		await this._userRepository.create({ email, roles: ["user"], name,passwordHash: hashedPassword});
    const otp = await this._otpService.generateOtp();

    const otpKey = `otp:${email}`;
    await this._cacheService.set(otpKey,otp,300)
    const message = {to: email, subject: OTP_SUBJECT, text: buildOtpEmailHtml(otp, "register") };
    await this._eventBus.publish('send.otp', message)
  }


	async verifyOtp(email: string, otp: string): Promise<{ user: UserDTO; accessToken: string; refreshToken: string }> {
		const otpKey = `otp:${email}`;
		const otpFromCache = await this._cacheService.get(otpKey);
		if (!otpFromCache) throw new AppError(ErrorMessage.OTP_NOT_FOUND, HttpStatus.NOT_FOUND);
		if (otpFromCache !== otp) throw new AppError(ErrorMessage.INVALID_OTP, HttpStatus.UNAUTHORIZED);
  ...
  async loginUser(email: string, password: string): Promise<{ accessToken: string; refreshToken: string, user: UserDTO }> {
    const user = await this._userRepository.findByEmail(email);
    if (!user || !user.isVerified) throw new AppError(ErrorMessage.USER_NOT_FOUND, HttpStatus.NOT_FOUND);
  ...
  async initiatePasswordReset(email: string): Promise<void> {
    ...
    if (!user) throw new AppError(ErrorMessage.USER_NOT_FOUND, HttpStatus.NOT_FOUND);
		const otp = await this._otpService.generateOtp();
		const otpKey = `resetotp:${email}`;
		await this._cacheService.set(otpKey,otp,300)
		const message = {to: email, subject: OTP_SUBJECT, text: buildOtpEmailHtml(otp,"reset") };
		await this._eventBus.publish('send.otp', message)
  }
