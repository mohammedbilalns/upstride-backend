...
import { IAuthService, IRegistrationService,IPasswordResetService } from "../../../domain/services";
...
export class AuthController {
  ...
  private setAuthCookies(
	...
	private setTokenCookie(res: Response, type:"reset"|"register", token: string) {
  ...
  verifyOtp = asyncHandler(async (req, res) => {
    ...
    const token  =	
      await this._registrationService.verifyOtp(email, otp);
  ...
  updatePassword = asyncHandler(async (req, res) => {
    ...
    await this._passwordResetService.updatePassword(email, newPassword, resetToken);
	...
	addInterests = asyncHandler(async (req, res) => {
		const { selectedAreas, selectedTopics,email } = addInterestsSchema.parse(req.body);
	
		const token = req.cookies.registertoken;
		await this._registrationService.createInterests(email, selectedAreas, selectedTopics, token);
