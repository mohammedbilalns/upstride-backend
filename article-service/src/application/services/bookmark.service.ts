...
export class BookMarkService implements IBookMarkService{
	constructor(
		private _bookMarkRepository : IBookMarkRepository
	){}

	async fetchBookMarkedArticles(userId: string, page: number, limit: number, sortBy?: string): Promise<{ articles: BookMark[]; total: number; }> {

		const {articles, total} = await this._bookMarkRepository.getBookMarkedArticles(userId, page, limit, sortBy)
	...
	async deleteBookMark(userId: string, articleId: string): Promise<void> {
		await this._bookMarkRepository.deleteByUserIdAndArticleId(userId, articleId)
	}
}
