...
export class NotificationController {
	constructor(private _notificationService: INotificationService) {}
		...
		markNotificationAsRead = asyncHandler( async (req, res) => {
			...
			const {notificationId} = markNotificationAsReadValidationSchema.parse(req.params)
		...
		fetchUserNotifications = asyncHandler( async (req, res) => {
			...
			const { notifications, total } = await this._notificationService.fetchUserNotifications(userId, page, limit);
