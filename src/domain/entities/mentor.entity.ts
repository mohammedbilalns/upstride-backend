...
export class Mentor {
	constructor(
		...
		if (this.experience.length > 7) {
			throw new Error("Maximum of 7 experience items allowed.");
		}
		if (this.applicationAttempts > 3) {
			throw new Error("Maximum of 3 application attempts allowed.");
		}
		if (this.areasOfExpertise.length > 2) {
			throw new Error("Maximum of 2 areas of expertise allowed.");
		}
	}
}
