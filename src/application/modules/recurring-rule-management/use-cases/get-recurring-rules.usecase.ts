...
import type { IGetRecurringRulesUseCase } from "./get-recurring-rules.usecase.interface";

@injectable()
export class GetRecurringRulesUseCase implements IGetRecurringRulesUseCase {
	constructor(
	...
	async execute({
		userId,
	}: GetRecurringRulesInput): Promise<GetRecurringRulesResponse> {
		...
		return { rules: availability?.recurringRules ?? [] };
	}
}
